<?
	...
	# Variables de diseño
	$title		= 'Estados de item';
	$name		= 'Estado de item';
	$titleForm	= 'Estados de item';
	$msgDelete	= '¿Esta seguro que desea eliminar este Estado de item?';
	...
	if ( in($action,'Agregar','Modificar') ) {
		if ( !validate::Text($nombre) ) 
			$error .= "<li>Debe ingresar un Estado de item.</li>";
...
?>				
		<tr> 
		    ...
		    <td width="490" class="cuadro1"><input name="nombre" type="text" value="<?=prepare_var( printif( $nombre, $reg->nombre ) )?>" style="width:100% " validate="str:Debe ingresar un Estado de item" maxlength="20" /></td>
		</tr>				
		<tr> 
