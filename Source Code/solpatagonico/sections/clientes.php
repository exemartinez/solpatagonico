<?
	...
	$name		= 'Cliente';
	$titleForm	= 'Clientes';
	$msgDelete	= '¿Esta seguro que desea eliminar este Cliente?';
	$msgUnDelete = '¿Esta seguro que desea restaurar este Cliente?';
	# Variables de programación
	$tabla		= "cliente";
	$campos		= "cuit,razonsocial,nroiibb,direccion,telefono";
	registrar( $campos.",action,id" );
	$id = intval( $id );
	...
	if ( in($action,'Agregar','Modificar') ) {
		...
		if ( $db->GetOne( "SELECT COUNT(*) FROM ".$tabla." WHERE cuit='".$cuit."'".( $action == 'Modificar' ? " AND id != '".$id."'" : '' ) ) ){
			$aux = $db->GetRow( "SELECT * FROM ".$tabla." WHERE cuit='".$cuit."'" );
			if( $aux['fecha_baja'] && validate::Date( substr( $aux['fecha_baja'], 0, 10 ) ) ){
				$error .= "<li>Este CUIT esta dado de baja logicamente. Si desea rehabilitarlo haga click ";
				$error .= "<a href=\"".$_SERVER['PHP_SELF']."?action=Rehabilitar&cuit=".$cuit."\">aqu&iacute;</a></li>";
			}
			else {
		...
		if ( $nroiibb && !validate::Integer($nroiibb) ) 
			$error .= "<li>Debe ingresar un Nro. de Ingresos Brutos v&aacute;lido (Solo n&uacute;meros).</li>";					
		if( $error != '' ){
			$action = 'frm'.$action;			
		}
	...
	if( $action == 'Rehabilitar' ){
		$ok = $db->Execute( "UPDATE ".$tabla." SET fecha_baja = '' WHERE cuit='".$cuit."'" );
		if( $ok ){
			$action = '';
			$error = '';
		} else {
			$action = '';
			$error = 'Error al Rehabilitar el cliente con cuit: '.$cuit;
		}
	}
...
<?
	if ( in($action,'','buscar') ) {
		$paginas = new pager(
			...
			FROM ".$tabla." 
			WHERE 1=1
				AND razonsocial LIKE '$nombre_list%' ".
			...
			<tr> 
				<td class="cuadro1izquierda" colspan="4">
					<table width="100%" border="0" cellspacing="0" cellpadding="3" class="text">
						<tr>
							<td><a href="<?=$_SERVER['PHP_SELF']?>?action=frmAgregar&cuit=<?=$cuit_list?>"><img 
							src="../sys_images/add.gif" border="0" align="absmiddle" /></a>&nbsp;<a 
							href="<?=$_SERVER['PHP_SELF']?>?action=frmAgregar&cuit=<?=$cuit_list?>">Agregar <?=$name?></a></td>
							<td>&nbsp;</td>
						</tr>
						<tr> 
							<td colspan="2">
...
?>
			...
			<tr>
				...
				<td class="titulos">Raz&oacute;n Social</td>
				<td width="80" class="titulos">Eliminado</td>
				<td width="100" class="titulos">&nbsp;</td>
			</tr>
<?
...
?>				
						<td width="20" align="center"><a href="sucursales.php?cliente_lst=<?=$reg->id?>"><img src="../sys_images/sucursales.gif" alt="Sucursales" border="0"></a></td>
						<td width="20" align="center"><a href="<?=$_SERVER['PHP_SELF']?>?action=frmModificar&id=<?=$reg->id?>"><img src="../sys_images/list.gif" alt="Modificar" border="0" /></a></td>
						<td width="20" align="center"><a href="<?=$_SERVER['PHP_SELF']?>?action=frmVer&id=<?=$reg->id?>"><img src="../sys_images/search.gif" alt="Ver" border="0" /></a></td>
<?
					if( 
						!$db->GetOne( "
							SELECT COUNT(*) 
							FROM pedido 
							WHERE id_cliente = '".$reg->id."' AND id_estado_pedido NOT IN ( 6, 7 ) 
						" )			
					){
?>
...
?>
		...
		<tr>
        	<td class="cuadro1izquierda">Nro. IIBB</td>
        	<td class="cuadro1">
<?
		if( $action == 'frmVer' ){
			echo printif( $reg->nroiibb );
		} else {
?>
			<input name="nroiibb" type="text" value="<?=prepare_var( printif( $nroiibb, $reg->nroiibb ) )?>" 
			style="width:100% " validate="norequired:int:Debe ingresar un Nro de IIBB (solo números)"/>
<?
		}
?>
			</td>
