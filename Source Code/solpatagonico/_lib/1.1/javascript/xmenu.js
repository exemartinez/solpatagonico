...
var webFXMenuHandler = {
	...
	overMenuItem	:	function (oItem) {
		...
		if (webfxMenuShowTime <= 0)
			this._over(jCFG_sitem);
		else
			//this.showTimeout = window.setTimeout(function () { webFXMenuHandler._over(jCFG_sitem) ; }, webfxMenuShowTime);
			// I hate IE5.0 because the piece of shit crashes when using setTimeout with a function object
			this.showTimeout = window.setTimeout("webFXMenuHandler._over(webFXMenuHandler.all['" + jCFG_sitem.id + "'])", webfxMenuShowTime);
	},
	outMenuItem	:	function (oItem) {
		...
		if (webfxMenuHideTime <= 0)
			this._out(jCFG_sitem);
		else
			//this.hideTimeout = window.setTimeout(function () { webFXMenuHandler._out(jCFG_sitem) ; }, webfxMenuHideTime);
			this.hideTimeout = window.setTimeout("webFXMenuHandler._out(webFXMenuHandler.all['" + jCFG_sitem.id + "'])", webfxMenuHideTime);
	},
	blurMenu		:	function (oMenuItem) {
		window.setTimeout("webFXMenuHandler.all[\"" + oMenuItem.id + "\"].subMenu.hide();", webfxMenuHideTime);
	},
	_over	:	function (jCFG_sitem) {
		top.status=jCFG_sitem.toolTip==''?jCFG_sitem.text:jCFG_sitem.toolTip;
		
		if (jCFG_sitem.subMenu) {
			jCFG_sitem.parentMenu.hideAllSubs();
			jCFG_sitem.subMenu.show();
		}
		else
			jCFG_sitem.parentMenu.hideAllSubs();
	},
	_out	:	function (jCFG_sitem) {
		// find top most menu
		var root = jCFG_sitem;
		var m;
...
WebFXMenu.prototype.show = function (relObj, sDir) {
	...
	if(ie){
		var iframeElement;
		if(iframeElement = document.getElementById('iframe_'+this.id)){
			iframeElement.style.left = opera ? this.left : this.left + "px";
			iframeElement.style.top = opera ? this.top : this.top + "px";
...
WebFXMenu.prototype.hide = function () {
	this.hideAllSubs();
	var iframeElement = document.getElementById('iframe_'+this.id);
	var divElement = document.getElementById(this.id);
	if(ie){
...
WebFXMenu.prototype.toString = function () {
	...
	var str=""; 
	if(ie){
		str += "<iframe id='iframe_" + this.id + "' frameborder='no' style='visibility: hidden; filter: alpha(opacity: 0); position:absolute; z-index:998;" + 
		"width:" + (!ieBox  ?
		...
		(ie50 ? "filter: none;" : "") + "'></iframe>";
	}
	
	str += "<div id='" + this.id + "' class='webfx-menu' style='" + 
	"width:" + (!ieBox  ?
		this.width - this.borderLeft - this.paddingLeft - this.borderRight - this.paddingRight  : 
		this.width) + "px;" +
	(this.useAutoPosition ? "left:" + this.left + "px;" + "top:" + this.top + "px;" : "") +
...
WebFXMenuButton.prototype.toString = function () {
	return	"<a" +
			...
			(webfxMenuUseHover ?
				...
				" onmouseout='webFXMenuHandler.outMenuItem(this)'") :
				(
					" onmouseover='status=\"" + (this.toolTip==''?this.text:this.toolTip) + "\"; return true;'" +
